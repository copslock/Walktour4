...
DLDTListData.prototype = {
	...
	renderToDoc:function(elecment){
		...
		content += '<h1><span class="dl-label">测试里程:</span><span class="dl-field">'+ dlToFixed(this.data.baseData.TestTotalMileage,1) +'m</span>';
		content += '&nbsp;&nbsp;<span class="dl-label">测试时长:</span><span class="dl-field">'+dlSecToTime(this.data.baseData.TestTotalTime)+'</span>';
		content += '<br/><span class="dl-label">覆盖率:</span><span class="dl-field">'+this.data.baseData.coverage+'</span></h1></li>';
		...
		else
		{
			...
			if (name == 'CS' || name == 'CSFB') {
				...
				for (var n = 0; n < tData.length; n++) {
					...
					html += '<tr><th class="dl-bus" colspan="3">'+DLVoiceNameEnum[subName]+'<span  class="dl-field">(次数:'+total+')</span></th></tr>';
			...
			else
			{
				...
				var html = '<li data-icon="false"><a href="detail.html?biz='+name+'" data-ajax="false"><table class="dl-border" rules="all"><thead><tr class="ui-bar-a"><th class="dl-bus" colspan="3">'+DLBusinessEnum[name]+'<span  class="dl-field">(次数:'+total+')</span></th></tr></thead>';
				html += '<tbody>';
				for (var i = 0; i < ggSteting.length; i++) {
					var f = ggSteting[i];
					html += '<tr><th class="dl-KPI" width="50%">'+f.display+'</th>';
