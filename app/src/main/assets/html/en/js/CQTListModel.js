...
DLCQTListData.prototype = {
	...
	dislplayBuilding:function(bName){
		...
		if (this.isByLog) {
			...
			content += 'File:'+tbName+'<br/>';
			content += 'Testing  Duration:'+baseInfo.TestTotalTime+'<br/>';
			content += 'Coverage Rate:'+baseInfo.coverage;
			content += '</h1>';
		}
		else
		{
		...
		}else
		{
			...
			if (name == 'CS' || name == 'CSFB') {
				...
				for (var i = 0; i < tData.length; i++) {
					...
					for (var n = 0; n < obj.data.length; n++) {
						...
						if (!this.isByLog) {
							html += '<tr><td colspan="3" class="dl-subBus"><span class="dl-label">'+ subName+'</span><span class="dl-field">(Attempts:'+total+')</span></td></tr>';
						}
						else
						{
							html += '<tr><td colspan="3" class="dl-subBus"><span class="dl-label">'+ DLVoiceNameEnum[obj.data[n].name]+'</span><span class="dl-field">(Attempts:'+total+')</span></td></tr>';
						}
			...
			else
			{
				...
				for (var i = 0; i < tData.length; i++) {
					...
					if (!this.isByLog) {
						html += '<thead>';
						html += '<tr><td colspan="3" class="dl-subBus"><span class="dl-label">'+ fName+'</span><span class="dl-field">(Attempts:'+total+')</span></td></tr>';
						html += '</thead>';
					}
					html +=  '<tbody>';
