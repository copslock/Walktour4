...
public class StringUtil {
	...
	/***
	 * 将日期格式2016-04-15 17:10:10 转化为微服务需要的格式
	 *
	 * @param str
	 * @return
	 */
	public static String changeDateTime(Context context, String str) throws Exception {
		String s = DateUtils.getCurrentDateTime();
		if (null == str || str.equals("") || null == s || s.equals(""))
			return "";
		String[] strd = str.split(" ");
		String[] strc = s.split(" ");
		if (strd[0].equals(strc[0])) {
			// 返回"今天 11:11"
		...
		String yesterday = new SimpleDateFormat(DateUtils.DATE_TIME_FORMAT, Locale.getDefault()).format(cal.getTime());
		strc = yesterday.split(" ");
		if (strd[0].equals(strc[0])) {
			// 返回"昨天 11:11"
		...
		cal1.getTime();
		String ws = dateFormater.format(cal1.getTime()) + "";
		// 本周最后一天
		cal1.set(Calendar.DAY_OF_WEEK, cal.getActualMaximum(Calendar.DAY_OF_WEEK));
		String we = dateFormater.format(cal1.getTime());
		if (ws.compareToIgnoreCase(strd[0]) <= 0 && we.compareToIgnoreCase(strd[0]) >= 0) {
			isThisWeek = true;
		}
		if (isThisWeek) {
			String weekString = "";
			final String dayNames[] = context.getResources().getStringArray(R.array.array_weekday);
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(DateUtils.DATETIME_FORMATTER.parse(str));
		...
		if (strs.length > 1) {
			strs = strs[0].split("-");
			if (strs.length == 3) {
				return strs[1] + "/" + strs[2] + "/" + strs[0].substring(2);
			}
		}
		return "";
	}
	...
	public static float formatTOFloat(String str){
		...
		try{
			...
			LogUtil.w("StringUtil",ex.getMessage());
		}
		return 0f;
	}
	/**
