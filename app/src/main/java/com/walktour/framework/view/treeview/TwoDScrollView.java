...
public class TwoDScrollView extends FrameLayout {
	 ...
	 * that watches for focus changes initiated outside this TwoDScrollView knows
	 ...
	 * (excluding fading edges) pretending that this View's top is located at the
	 * parameter top.
	 *
	 * @param topFocus
	 *          look for a candidate is the one at the top of the bounds if
	 *          topFocus is true, or at the bottom of the bounds if topFocus is
	 *          false
	 * @param top
	 ...
	 *          the View that has highest priority and will be returned if it is
	...
	private View findFocusableViewInMyBounds(final boolean topFocus, final int top, final boolean leftFocus,
		...
		/*
		 * The fading edge's transparent side should be considered for focus since
		 * it's mostly visible, so we divide the actual fading edge length by 2.
		 */
	...
	private View findFocusableViewInBounds(boolean topFocus, int top, int bottom, boolean leftFocus, int left,
		 ...
		 * top, and its bottom is above the bound's bottom. A partially contained
		 * focusable is one where some part of it is within the bounds, but it also
		 * has some part that is not within bounds. A fully contained focusable is
				...
				if (focusCandidate == null) {
					...
					if (foundFullyContainedFocusable) {
						if (viewIsFullyContained && viewIsCloserToVerticalBoundary && viewIsCloserToHorizontalBoundary) {
							/*
							 * We're dealing with only fully contained views, so it has to be
							 * closer to the boundary to beat our candidate
							 */
							focusCandidate = view;
						}
					} else {
						if (viewIsFullyContained) {
							...
							/*
							 * Partially contained view beats another partially contained view
							 * if it's closer
							 */
							focusCandidate = view;
						}
	 ...
	 *
	 * @param direction
	 *          the scroll direction: {@link android.view.View#FOCUS_UP} to go the
	 *          top of the view or {@link android.view.View#FOCUS_DOWN} to go the
	 *          bottom
...
//	}
	...
	/**
	 * If rect is off screen, scroll just enough to get it (or at least the first
	 ...
	 *          that the finger/curor is moving down the screen, which means we
