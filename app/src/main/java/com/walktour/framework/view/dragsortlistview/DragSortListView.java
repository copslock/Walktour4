package com.walktour.framework.view.dragsortlistview;
...
public class DragSortListView extends ListView {
	...
	public DragSortListView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			mSlideRegionFrac = Math.max(0.0f,
					Math.min(1.0f, 1.0f - a.getFloat(R.styleable.DragSortListView_slide_shuffle_speed, 0.75f)));
			...
			if (useDefault) {
				...
				DragSortController controller = new DragSortController(this, dragHandleId, dragInitMode, removeMode,
	 ...
	 * @param top
	 * @param height
	 *          Height of item at position. If -1, this function calculates this
	 *          height.
	 *
	...
	private abstract class SmoothAnimator implements Runnable {
		...
		public abstract void onUpdate(float frac, float smoothFrac);
	 ...
	 *          position in your input ListAdapter).
	 * @param to
	 *          Target position (NOTE: headers/footers ignored! this is a position
	 *          in your input ListAdapter).
	 */
	 ...
	 *
	 * @param upperFrac
	 *          Fraction of ListView height for up-scroll bound. Capped at 0.5f.
	 * @param lowerFrac
	...
	public interface FloatViewManager {
		 ...
		 *
		 * @param position
		 *          Position of item to drag (NOTE: <code>position</code> excludes
		 *          header Views; thus, if you want to call
		 ...
		 *          The floating View.
		 * @param location
		 *          The location (top-left; relative to DSLV top-left) at which the
		 *          float View would like to appear, given the current touch
		 *          location and the offset provided in
		 ...
		 *
		 * @param floatView
		 *          The floating View passed to {@link #onCreateFloatView(int)}.
		 */
	 ...
	 * move the check state from the item's old position to the new position 钬? *
	...
	public interface DragScrollProfile {
		 ...
		 * 
		 * @param w
		 *          Normalized position in scroll region (i.e. w \in [0,1]). Small w
		 *          typically means slow scrolling.
		 * @param t
		 *          Time (in milliseconds) since start of scroll (handy if you want
		 *          scroll acceleration).
	...
	private class DragSortTracker {
		...
		public void appendState() {
			...
			mBuilder.append("<FirstExpBlankHeight>").append(getItemHeight(mFirstExpPos) - getChildHeight(mFirstExpPos))
