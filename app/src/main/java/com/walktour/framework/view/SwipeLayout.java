...
public class SwipeLayout extends FrameLayout {
	...
	public static interface SwipeDenier {
		/*
		 * Called in onInterceptTouchEvent Determines if this swipe event should be
		 * denied Implement this interface if you are using views with swipe
	...
	private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
		...
		public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
			...
			int evLeft = surfaceView.getLeft(), evRight = surfaceView.getRight(), evTop = surfaceView.getTop(),
	 ...
	 * makes the view may not always get the event when the view is totally show(
	 * fraction = 1), so , we need to calculate every time.
	 */
	...
	protected boolean isViewShowing(Rect relativePosition, DragEdge availableEdge, int surfaceLeft, int surfaceTop,
	...
	protected void dispatchRevealEvent(final int surfaceLeft, final int surfaceTop, final int surfaceRight,
		...
		for (Map.Entry<View, ArrayList<OnRevealListener>> entry : mRevealListeners.entrySet()) {
			...
			if (isViewShowing(rect, mCurrentDragEdge, surfaceLeft, surfaceTop, surfaceRight, surfaceBottom)) {
				mShowEntirely.put(child, false);
				int distance = 0;
				float fraction = 0f;
				if (getShowMode() == ShowMode.LayDown) {
			...
			if (isViewTotallyFirstShowed(child, rect, mCurrentDragEdge, surfaceLeft, surfaceTop, surfaceRight,
					surfaceBottom)) {
				mShowEntirely.put(child, true);
				for (OnRevealListener l : entry.getValue()) {
					if (mCurrentDragEdge == DragEdge.Left || mCurrentDragEdge == DragEdge.Right)
	...
	/**
	 * {@link android.view.View.OnLayoutChangeListener} added in API 11. I need to
	 * support it from API 8.
	 */
	public interface OnLayout {
		public void onLayout(SwipeLayout v);

		public void layout(SwipeLayout v);
	}
		...
		if (gravity > 0) {
			...
			for (Map.Entry<DragEdge, View> entry : mDragEdges.entrySet()) {
				if (entry.getValue() == null) {
					// means used the drag_edge attr, the no gravity child should be use
					// set
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mOnLayoutListeners != null)
			for (int i = 0; i < mOnLayoutListeners.size(); i++) {
				...
				mOnLayoutListeners.get(i).layout(this);
			}
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (!beforeCheck && mIsBeingDragged) {
				// let children has one chance to catch the touch, and request the swipe
				// not intercept
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			// the drag state and the direction are already judged at
	...
	public boolean isLeftSwipeEnabled() {
		...
		return bottomView != null && bottomView.getParent() == this && bottomView != getSurfaceView()
	...
	/**
	 * set the drag distance, it will force set the bottom view's width or height
	 * via this value.
	 *
	 * @param max
	...
	/**
	 * @return all bottomViews: left, top, right, bottom (may null if the edge is
	 *         not set)
	 */
	...
	/**
	 * Deprecated, use {@link #addDrag(DragEdge, View)} When using multiple drag
	 * edges it's a good idea to pass the ids of the views that you're using for
	 * the left, right, top bottom views (-1 if you're not using a particular
	 * view)
	 */
	@Deprecated
