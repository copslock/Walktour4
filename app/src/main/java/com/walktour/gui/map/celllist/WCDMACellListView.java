 ...
 *
 */
public class WCDMACellListView extends BaseCellListView {

	public WCDMACellListView(Context context) {
		super(context, "WCDMA", BaseStation.NETTYPE_WCDMA, -25, -124);
	}

	public WCDMACellListView(Context context, AttributeSet attrs) {
		super(context, attrs, "WCDMA", BaseStation.NETTYPE_WCDMA, -25, -124);
	}

	@Override
	protected void parseCellValues(TraceInfoData traceData) {
		String[] neighbors = getParaValue(UnifyParaID.W_TUMTSCellInfoV2).split(";");
		this.cellList.clear();
		for (String neighbor : neighbors) {
			...
			if (values.length >= 6) {
				...
				cellParam.tableValues = new String[] { super.getInt(values[1]), super.getInt(values[2]), values[4],
						UtilsMethodPara.getWcdmaSetType(values[0]) };
				cellParam.histogramValue = super.getDouble(values[5]);
				cellParam.type = UtilsMethodPara.getWcdmaSetType(values[0]);
	...
	protected String[] getHeaders() {
		return new String[] { "UARFCN", "PSC", "Ec/Io", "Set", "RSCP" };
	}
