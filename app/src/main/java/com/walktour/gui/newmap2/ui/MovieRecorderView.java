...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
    ...
    private void createRecordDir() {
        //录制的视频保存文件夹
        File sampleDir = new File(WalktourConst.SAVE_ATTACH_DIR);//录制视频的保存地址
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        ...
        // 创建文件
        mVecordFile = new File(WalktourConst.SAVE_ATTACH_DIR +File.separator+ df.format(new Date()) + ".flv");// mp4格式的录制的视频文件
    }
    ...
    private void initRecord() throws IOException {
        ...
        mMediaRecorder.setOutputFormat(OutputFormat.DEFAULT);// 视频输出格式
