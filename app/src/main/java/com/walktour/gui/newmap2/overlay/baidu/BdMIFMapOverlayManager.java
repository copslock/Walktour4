 ...
 * @describe mif图层
 */
public class BdMIFMapOverlayManager extends BaseOverlayManager {

    private static final String TAG = BdMIFMapOverlayManager.class.getSimpleName();
     ...
     * 基站详情弹出框
     */
    private List<Overlay> mMifOverlay = new ArrayList<>();
    ...
    public BdMIFMapOverlayManager(Context context) {
        super(context);
    ...
    private void setMapTypeAndOverlook(int mapType, int overlook) {
        getMapControllor().setMapType(mapType);
        MapStatus mapStatus = getMapControllor().getMapStatus();
        MapStatus newStatus = new MapStatus.Builder(mapStatus).overlook(overlook).build();
        getMapControllor().animateMapStatus(MapStatusUpdateFactory.newMapStatus(newStatus));
    }
    @Override
    ...
    public boolean clearOverlay() {
        for (Overlay overlay : mMifOverlay) {
            overlay.remove();
        }
        mMifOverlay.clear();
    ...
    public Overlay drawLine(List<MyLatLng> points) {
        List<LatLng> list = new ArrayList<>();
        for (MyLatLng latlng : points) {
            list.add(BaiduMapUtil.convert(latlng.latitude, latlng.longitude));
        }
        OverlayOptions ooPolyline = new PolylineOptions().width(5)
                .color(0xAAFF0000).points(list);
        Overlay line=null;
        if (this.getMapControllor()!=null){
             line=this.getMapControllor().addOverlay(ooPolyline);
            mMifOverlay.add(line);
        }
        return line;
    }

    public Overlay drawText(MyLatLng latlng, String text, int textSize, int textColor) {
        ...
        if (this.getMapControllor()!=null){
            textOverlay=this.getMapControllor().addOverlay(options);
            mMifOverlay.add(textOverlay);
        }
        return textOverlay;
    }
}
