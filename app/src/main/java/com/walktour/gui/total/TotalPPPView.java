 ...
 */
public class TotalPPPView  extends BasicTotalView {
	
	private static String tag = "TotalPPPView";
	private boolean isRegisterReceiver = false;

	public TotalPPPView(Context context) {
		super(context);
	}
	
	public TotalPPPView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	protected Bitmap CreateTable(Canvas cv) {
		...
		/* 第一行 */
		paraname = getContext().getString(R.string.total_ppp);
	...
	protected Bitmap CreateTableData(Canvas cv, HashMap<String, Long> hMap) {
		...
		value = ""+ TotalDataByGSM.getHashMapValue(hMap, 
						TotalStruct.TotalPPP._pppRequest.name());// 发送尝试次数
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
				rowHeight * 2 - rowUpBit, paramPaint);
		
		value = "" + TotalDataByGSM.getHashMapValue(hMap,
						TotalStruct.TotalPPP._pppSuccess.name());// 发送成功次数
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
				rowHeight * 3 - rowUpBit, paramPaint);
		
		value = "" + TotalDataByGSM.getHashMapMultiple(hMap,
						TotalStruct.TotalPPP._pppSuccess.name(),
						TotalStruct.TotalPPP._pppRequest.name(),
						100, "%");// 成功率
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
		...
		value = "" + TotalDataByGSM.getHashMapMultiple(hMap,
						TotalStruct.TotalPPP._pppDelay.name(),
						TotalStruct.TotalPPP._pppSuccess.name(),
						1, "");// 平均时延
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
	...
	private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(TotalDataByGSM.TotalResultToPicture)) {
				...
				TotalPPPView.this.buildDrawingCache();
				UtilsMethod.SaveBitmapToFile(
						TotalPPPView.this.getDrawingCache(), path);
			} else {
				invalidate();
			}
		}
