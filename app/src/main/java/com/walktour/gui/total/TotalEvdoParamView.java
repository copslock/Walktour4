 ...
 *
 */
public class TotalEvdoParamView extends BasicTotalView {
	private static String tag = "TotalParaView";
	private boolean isRegisterReceiver = false;
	DisplayMetrics metric = new DisplayMetrics();

	public TotalEvdoParamView(Context context) {
		super(context);
	...
	public TotalEvdoParamView(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	protected Bitmap CreateTable(Canvas cv) {
		...
		cv.drawText(paraname,
				...
				rowHeight * 2 - rowUpBit + marginSize, fontPaint);
		paraname = getResources().getString(R.string.evdo_rx_agc0);// "RxQualFull";
		cv.drawText(paraname,
				colsWidth * 0 + (colsWidth * 2 - fontPaint.measureText(paraname))
						/ 2, rowHeight * 3 - rowUpBit + marginSize, fontPaint);
		paraname = getResources().getString(R.string.evdo_rxAGC1);// "RxQualSub";
		cv.drawText(paraname,
				colsWidth * 0 + (colsWidth * 2 - fontPaint.measureText(paraname))
						/ 2, rowHeight * 4 - rowUpBit + marginSize, fontPaint);
		paraname = getResources().getString(R.string.evdo_txAgc);// "RxLevFull";
		cv.drawText(paraname,
				colsWidth * 0 + (colsWidth * 2 - fontPaint.measureText(paraname))
						/ 2, rowHeight * 5 - rowUpBit + marginSize, fontPaint);
		paraname = getResources().getString(R.string.evdo_total_sinr);// "RxLevBub";
		cv.drawText(paraname,
				colsWidth * 0 + (colsWidth * 2 - fontPaint.measureText(paraname))
	...
	protected Bitmap CreateTableData(Canvas cv,
		...
		TotalMeasureModel data = TotalDataByGSM.getHashMapMeasure(hMap,
				TotalStruct.TotalMeasurePara._eRXAGC0.name());
		...
		data = TotalDataByGSM.getHashMapMeasure(hMap,
				TotalStruct.TotalMeasurePara._eRXAGC1.name());
		value = "" + (data.getMaxValue() != -9999 ? data.getMaxValue() : "");
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
		...
		data = TotalDataByGSM.getHashMapMeasure(hMap,
				TotalStruct.TotalMeasurePara._eTxAGC.name());
		value = "" + (data.getMaxValue() != -9999 ? data.getMaxValue() : "");
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
		...
		data = TotalDataByGSM.getHashMapMeasure(hMap,
				TotalStruct.TotalMeasurePara._eTotalSINR.name());
		value = "" + (data.getMaxValue() != -9999 ? data.getMaxValue() : "");
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
	...
	private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			 if (intent.getAction().equals(
				...
				TotalEvdoParamView.this.buildDrawingCache();
				UtilsMethod.SaveBitmapToFile(
						TotalEvdoParamView.this.getDrawingCache(), path);
			} else {
				invalidate();
			}
		}
