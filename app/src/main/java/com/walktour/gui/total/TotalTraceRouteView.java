 ...
 * DNS统计界面
 */
public class TotalTraceRouteView  extends BasicTotalView {
	
	private static String tag = "TotalTraceRouteView";
	private boolean isRegisterReceiver = false;

	public TotalTraceRouteView(Context context) {
		super(context);
	}
	
	public TotalTraceRouteView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	protected Bitmap CreateTable(Canvas cv) {
		...
		/* 第一行 */
		paraname = getContext().getString(R.string.total_traceroute);
		...
		paraname = getContext().getString(R.string.total_traceroute_hotcount);
		cv.drawText(paraname,colsWidth * 0 + (colsWidth * 2 - fontPaint.measureText(paraname))/ 2, 
	...
	protected Bitmap CreateTableData(Canvas cv, HashMap<String, Long> hMap) {
		...
		value = ""+ TotalDataByGSM.getHashMapValue(hMap, 
						TotalStruct.TotalTraceRoute._traceRouteTrys.name());// 发送尝试次数
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
				rowHeight * 2 - rowUpBit, paramPaint);
		
		value = "" + TotalDataByGSM.getHashMapValue(hMap,
						TotalStruct.TotalTraceRoute._traceRouteSucc.name());// 发送成功次数
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
				rowHeight * 3 - rowUpBit, paramPaint);
		
		value = "" + TotalDataByGSM.getHashMapMultiple(hMap,
						TotalStruct.TotalTraceRoute._traceRouteSucc.name(),
						TotalStruct.TotalTraceRoute._traceRouteTrys.name(),
						100, "%");// 成功率
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
		...
		value = "" + TotalDataByGSM.getHashMapMultiple(hMap,
						TotalStruct.TotalTraceRoute._traceRouteDelay.name(),
						TotalStruct.TotalTraceRoute._traceRouteSucc.name(),
						1, "");// 平均时延
		cv.drawText(value,
				colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
		...
		value = ""+ TotalDataByGSM.getHashMapValue(hMap, 
				TotalStruct.TotalTraceRoute._traceRouteHopCounts.name());// 发送尝试次数
		cv.drawText(value,
		colsWidth * 2 + (colsWidth - paramPaint.measureText(value)) / 2,
	...
	private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(TotalDataByGSM.TotalResultToPicture)) {
				...
				TotalTraceRouteView.this.buildDrawingCache();
				UtilsMethod.SaveBitmapToFile(
						TotalTraceRouteView.this.getDrawingCache(), path);
			} else {
				invalidate();
			}
		}
