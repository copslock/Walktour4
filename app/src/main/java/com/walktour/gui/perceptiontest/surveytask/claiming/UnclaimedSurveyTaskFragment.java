...
public class UnclaimedSurveyTaskFragment extends Fragment {

    public static final String SPF_NAME = "survey_task_spf";
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_unclaimed_survey_task, container, false);
        ...
        mRecyclerAdapter.replaceList(DataProvider.getInstance().getSurveyTaskList(getActivity(), SurveyTask.STATE_UNCLAIMED));
        mRecyclerAdapter.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent(getActivity(), ClaimingSurveyTaskDetailActivity.class);
                SurveyTask surveyTask = mRecyclerAdapter.getList().get(position);
                intent.putExtra(ClaimingSurveyTaskDetailActivity.EXTRA_SURVEY_TASK, surveyTask);
    ...
    @OnClick(R.id.btn_obtain_survey_task)
    public void onClickObtainSurveyTask() {
        if (!mProgressDialog.isShowing()) {
            mProgressDialog.show();
            mHandler.sendEmptyMessageDelayed(MSG_STOP_PD, 3000);
        }
    }
    ...
    public void updateLocation(LocationEvent event) {
        SharedPreferences.Editor edit = getActivity().getSharedPreferences(SPF_NAME, Context.MODE_PRIVATE).edit();
        edit.putString("survey_lat", event.lat + "").putString("survey_lng", event.lng + "").apply();
