 ...
 * @date 2019/3/19
 */
public class MosStableActivity extends FragmentActivity implements MosCallback<String>{
    ...
    protected void onResume() {
        super.onResume();
        BluetoothMOSDevice mocDevice = BluetoothMOSFactory.get().getCurrMOCDevice();
        if (mocDevice != null) {
            mTvMosMac.setText(mocDevice.getAddress());
        }
    }

    @OnClick(R.id.btn_match_mos)
    void clickMatchMosButton() {
        Intent intent = new Intent(this, MosMatchActivity.class);
    ...
    void clickBeginCaculate() {
        ...
        if (StringUtil.isEmpty(dir8K)) {
            ...
            return;
        } else if ("未连接".equals(mTvMosMac.getText().toString())) {
            ToastUtil.showShort(this, "请先连接蓝牙Mos头");
            return;
        }
        ...
        MosstableDialog dialog = new MosstableDialog();
