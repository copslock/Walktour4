...
public class TaskWapRefurbish extends BaseTaskActivity {
    ...
    private EditText refurbishEditText;
        	...
        	if(RABTAG.equals(super.getRabTag())){
        		for(int i=0;i<taskd.getCurrentTaskList().size();i++){ 
        			if(taskd.getCurrentTaskList().get(i).getTaskID().equals(super.getModifyBefRabName())){
        				model =	(TaskWapPageModel)((TaskRabModel)(taskd.getCurrentTaskList().get(i))).getTaskModel().get(taskListId);
        				break;
        			}
        		}
        	}else{
	...
	private void showView(){
		...
		final View textEntryView = factory.inflate(R.layout.task_waprefurbish, null); 
		((TextView)textEntryView.findViewById(R.id.title_txt)).setText(R.string.act_task_waprefurbish);//设置标题
		...
		TextView tv_refreshcount =(TextView)textEntryView.findViewById(R.id.txt_refreshcount);
		TextView tv_refurbish =(TextView)textEntryView.findViewById(R.id.txt_refurbish);
        ...
        tv_timeOut.setText(getString(R.string.task_refresh_timeout));
        tv_showPage.setText(getString(R.string.task_showPage));
        tv_showPage.setVisibility( View.GONE );//暂时屏蔽显示网页的功能
        tv_refreshcount.setText(getString(R.string.task_refreshcount));
        tv_refreshcount.setVisibility( View.GONE );//下载的测试计划中没有包含刷新次数，先屏蔽
        tv_refurbish.setText(getString(R.string.task_refurbish));
		...
		refreshcountEditText =(EditText)textEntryView.findViewById(R.id.edit_refreshcount);
		refreshcountEditText.setVisibility( View.GONE );//下载的测试计划中没有包含刷新次数，先屏蔽
		refurbishEditText =(EditText)textEntryView.findViewById(R.id.edit_refurbish);
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
                R.layout.simple_spinner_custom_layout, getResources().getStringArray(R.array.public_switch));
        ...
        if(model!=null){
	        ...
	        refreshcountEditText.setText( String.valueOf(model.getRefreshcount()));
	        refurbishEditText.setText( String.valueOf(model.getRefreshDepth()));
        	...
        	taskNameEditText.setText("WAP Refresh");
	        ...
	        refreshcountEditText.setText("1");
	        refurbishEditText.setText("3");
    ...
    public void saveTestTask() {
        if(taskNameEditText.getText().toString().trim().length()==0){    //任务名为空
            ...
            return;
        }else if( repeatEditText.getText().toString().trim().equals("0") ||  repeatEditText.getText().toString().trim().length()==0){
			...
			return;
        }else if( interValEditText.getText().toString().trim().equals("0") || interValEditText.getText().toString().trim().length()==0 ){
            ...
            portEditText.setError(getString(R.string.task_alert_nullPort));
            return;
        }else if( !(Verify.isUrl(urlEditText.getText().toString().trim())) ){//URL地址
            ...
            return;
        }else if(timeOutEditText.getText().toString().equals("") || Integer.valueOf(timeOutEditText.getText().toString()) < 5 || Integer.valueOf(timeOutEditText.getText().toString())> 300){//超时
            Toast.makeText(getApplicationContext(), R.string.alert_inputagain, Toast.LENGTH_SHORT).show();
            timeOutEditText.setError(getString(R.string.alert_inputagain) + "  [5,300]");
            return;
            ...
            interValEditText.setError(getString(R.string.task_alert_nullInterval));
            return;
        }else if(  refurbishEditText.getText().toString().trim().length()==0         //刷新层数
                || Integer.valueOf( refurbishEditText.getText().toString().trim() )<=0  ){
            Toast.makeText( getApplicationContext(), getString(R.string.task_refurbish)+getString(R.string.task_alert_zore), Toast.LENGTH_SHORT).show();
            refurbishEditText.setError( getString(R.string.task_refurbish)+getString(R.string.task_alert_zore));
            return;
        }else if( Integer.valueOf( repeatEditText.getText().toString().trim() )<=0  ) {//重复次数必须大于1
            Toast.makeText( getApplicationContext(), getString(R.string.task_repeat)+getString(R.string.task_alert_zore), Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText( getApplicationContext(), getString(R.string.task_alert_input)+getString(R.string.task_gateway), Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText( getApplicationContext(), getString(R.string.task_alert_input)+getString(R.string.task_port), Toast.LENGTH_SHORT).show();
            portEditText.setError(getString(R.string.task_alert_input)+getString(R.string.task_port));
            return;
        }
        
        if(taskNameEditText.getText().toString().trim().length()==0){    //任务名为空
            Toast.makeText(com.walktour.gui.task.TaskWapRefurbish.this.getApplicationContext(), R.string.task_alert_nullName, Toast.LENGTH_SHORT).show();
            taskNameEditText.setError(getString(R.string.task_alert_nullName));
            return;
        }else{
            ...
            for(int i=0;i<taskNames.length;i++){
                if((isNew && taskNames[i].equals(taskNameEditText.getText().toString().trim()))  //判断任务名是否重复
                        || (!isNew && taskListId !=i && taskNames[i].equals(taskNameEditText.getText().toString().trim())))
                {
...
//                    return;
        ...
        if(model==null){
            model = new TaskWapPageModel(WalkStruct.TaskType.WapRefurbish.name());
      ...
      //依据标记区分用户名的编辑
        ...
        model.setRefreshcount(Integer.parseInt( refurbishEditText.getText().toString().trim().length()==0?"0":refurbishEditText.getText().toString().trim()));
        //这里屏蔽了刷新次数，设定为刷新次数和刷新层数一样
        model.setRefreshDepth(Integer.parseInt( refurbishEditText.getText().toString().trim().length()==0?"0":refurbishEditText.getText().toString().trim()));
        model.setUrl(urlEditText.getText().toString().trim().length()==0?"0":urlEditText.getText().toString().trim());
        model.setGateway(gatewayEditText.getText().toString().trim().length()==0?"0":gatewayEditText.getText().toString().trim());
        model.setPort(Integer.parseInt( portEditText.getText().toString().trim().length()==0?"0":portEditText.getText().toString().trim()));
        model.setInterVal(Integer.parseInt( interValEditText.getText().toString().trim().length()==0?"0":interValEditText.getText().toString().trim()));
        ...
        if(RABTAG.equals(super.getRabTag())){//依标志区分并发与普通业务
        	for (int i = 0; i < taskd.getCurrentTaskList().size(); i++) {
        		if((isNew?super.getMultiRabName():super.getModifyBefRabName()).equals(taskd.getCurrentTaskList().get(i).getTaskName())){
    ...
    public void addEditTextWatcher(){
        ...
        urlEditText.addTextChangedListener(new EditTextWatcher() {
            @Override
            public void afterTextChanged(Editable s) {
            	if( !( Verify.isUrl(urlEditText.getText().toString().trim())) ){//URL地址
                    urlEditText.setError(getString(R.string.task_alert_nullUrl));
                }
            }
        });
        ...
        timeOutEditText.addTextChangedListener(new EditTextWatcher() {
            @Override
            public void afterTextChanged(Editable s) {
            	String valueString = timeOutEditText.getText().toString();
                if(valueString.equals("") || Integer.valueOf(valueString) < 0 || Integer.valueOf(valueString)> 300){
                    timeOutEditText.setError(getString(R.string.alert_inputagain) +  " [5,300]");
                }
            }
        });
