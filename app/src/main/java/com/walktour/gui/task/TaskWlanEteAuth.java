 ...
 * 
 */
public class TaskWlanEteAuth extends BaseTaskActivity {
	TaskListDispose taskd = null;
	TaskWlanEteAuthModel model = null;
	...
	private Context context=TaskWlanEteAuth.this;
			...
			if (RABTAG.equals(super.getRabTag())) {
				for (int i = 0; i < taskd.getCurrentTaskList().size(); i++) {
					if (taskd.getCurrentTaskList().get(i).getTaskID().equals(super.getMultiRabName())) {
						model = (TaskWlanEteAuthModel) ((TaskRabModel) (taskd.getCurrentTaskList().get(i)))
								.getTaskModel().get(taskListId);
						break;
					}
				}
			} else {
				model = (TaskWlanEteAuthModel) taskd.getTaskListArray().get(taskListId);
			}
			abstModel = model;
			isNew = false;
		}
	...
	private void findView() {
		setContentView(R.layout.task_wlaneteauth);
		(initTextView(R.id.title_txt)).setText(R.string.act_task_wlaneteauth);// 设置标题
		...
		tv_timeOut.setText(getString(R.string.task_wlaneteauth_timeout));
		...
		if (model != null) {
			...
			taskNameEditText.setText(getString(R.string.act_task_wlaneteauth)+"");
	...
	private void initWifiInfo(){
		if (null != model) {
			String apName = model.getWlanETEAuthTestConfig().getApName();
			...
			wifiUserET.setText(model.getWlanETEAuthTestConfig().getWlanAccount().getUsername()+"");
			wifiPasswordET.setText(model.getWlanETEAuthTestConfig().getWlanAccount().getPassword()+"");
		}
	}
	...
	public void saveTestTask() {
		if (taskNameEditText.getText().toString().trim().length() == 0) { // 任务名为空
			Toast.makeText(com.walktour.gui.task.TaskWlanEteAuth.this.getApplicationContext(),
		...
		if (model == null){
			model = new TaskWlanEteAuthModel();
		...
		if (((Long) dataConnectType.getSelectedItemId()).intValue() == 1) // 只有是WIFI的才设置
		  ...
		  if(((Long)dataConnectType.getSelectedItemId()).intValue()==1){
	        	model.setTypeProperty(WalkCommonPara.TypeProperty_Wlan);
	        	model.getWlanETEAuthTestConfig().setApName(wifiSSIDET.getText().toString()+"");
	        	model.getWlanETEAuthTestConfig().getWlanAccount().setUsername(wifiUserET.getText().toString()+"");
	        	model.getWlanETEAuthTestConfig().getWlanAccount().setPassword(wifiPasswordET.getText().toString()+"");
	        }
