...
import com.walktour.Utils.SFTPUtils;
 ...
 * @author weirong.fan
 */
public class SFtpListActivity extends BasicActivity implements OnItemClickListener, OnClickListener {
    public ListView mListView;

    private CustomListAdapter customadapter = null;

    private List<LsEntry> files = new ArrayList<LsEntry>();
    ...
    private AbstractMap<String, List<LsEntry>> dataCache;

    private LsEntry ftpFile;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dataCache = new ConcurrentHashMap<String, List<LsEntry>>();
        dirList.add("");   //加入默认路径
    ...
    private void setTitle() {
        ...
        title.setText("sftp://" + ftpServerModel.getIp() + filePath);
    }
    ...
    private void showDialog() {
        ftpDialog = new ProgressDialog(SFtpListActivity.this);
    ...
    class getFtpList extends AsyncTask {
        ...
        protected Object doInBackground(Object... params) {
            ...
            files.clear();
            if (cutFtp == null) {
                cutFtp = new SFTPUtils(ftpServerModel.getIp(),Integer.parseInt(ftpServerModel.getPort()),ftpServerModel.getLoginUser(),ftpServerModel.getLoginPassword());
                sftp=cutFtp.connect();
            }
            try {
                if (!sftp.isConnected()) {
            ...
            if (sftp.isConnected()) {
                try {
                    if (count == 1) {
                        dirList.remove(0);
                        try {
                            String ftpWorkDir = sftp.getHome();
                    ...
                    TreeSet<LsEntry> fileTree = null;

                    fileTree = new TreeSet<LsEntry>(
                            new Comparator<Object>() {
                                ...
                                public int compare(Object objFile1, Object objFile2) {
                                    if (objFile1 == null)
                                        ...
                                        LsEntry file1 = (LsEntry) objFile1;
                                        LsEntry file2 = (LsEntry) objFile2;
//                                        if (file1.getAttrs().get != file2.getType())
...
//                                        else
                    ...
                    Vector<LsEntry> listFile = cutFtp.listFiles(filedir.toString());
                    Log.i("ftp list ", listFile.size() + "");
                    if (listFile != null && listFile.size() > 0) {
                        // 如果模板浏览类型为Upload
                        for (int i = 0; i < listFile.size(); i++) {
                            ftpFile = listFile.get(i);
                            if (!ftpFile.getFilename().equals(".") && !ftpFile.getFilename().equals("..")) {
                        ...
                        for (LsEntry file : fileTree) {
                            files.add(file);
                        }

                        System.out.println("目录路径" + filedir.toString());
                        if (!filedir.toString().equals("/")) {
//                            ChannelSftp sss=new ChannelSftp();
//                            ftpFile = new sss..LsEntry(".." + getString(R.string.up_directory),"",null);
////                            ftpFile.setName(".." + getString(R.string.up_directory));
////                            ftpFile.setType(LsEntry.DIRECTORY_TYPE);
//                            files.add(0, ftpFile);
                        }
                    }
                } catch (Exception e) {
    ...
    class CustomListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            LsEntry ftp = files.get(position);

            long size = ftp.getAttrs().getSize() / 1000;
            String measure = size > 0 ? "###,###KB" : "###,###B";
            size = size > 0 ? size : ftp.getAttrs().getSize();


            if (ftp.getAttrs().isReg()) {
                if (ftp.getFilename().endsWith(".rar") || ftp.getFilename().endsWith(".zip")) {
            ...
            viewHolder.ItemTitle.setText(ftp.getFilename());
            return convertView;
        }
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int index, long arg3) {
        if (index == 0 && !dirList.toString().equals("[/]")) {
            ...
            if(dirList.size()>0) {
                LsEntry ftpf = files.get(index);
                if (ftpf.getAttrs().isReg()) {
                    StringBuffer fdir = new StringBuffer();
                    for (int i = 0; i < dirList.size(); i++) {
                        fdir.append(dirList.get(i));
                    }

                    Intent data = getIntent().putExtra("path", fdir.append(fdir.toString().endsWith("/") ? (files.get(index).getFilename().toString()) : "/" + files.get(index).getFilename()).toString());
                    finish();
                    SFtpListActivity.this.setResult(RESULT_OK, data);
                } else if (ftpf.getAttrs().isDir()) {
                    dirList.add(ftpf.getFilename() + "/");
                    new getFtpList().execute();
                } else {
