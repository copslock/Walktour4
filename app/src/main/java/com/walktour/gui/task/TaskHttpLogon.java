...
public class TaskHttpLogon extends BaseTaskActivity {
	...
	private Context context=TaskHttpLogon.this;
    ...
    public void saveTestTask() {
        if(et_taskName.getText().toString().trim().length()==0){    //任务名为空
            Toast.makeText(com.walktour.gui.task.TaskHttpLogon.this.getApplicationContext(), R.string.task_alert_nullName, Toast.LENGTH_SHORT).show();
            return;
            ...
            Toast.makeText(com.walktour.gui.task.TaskHttpLogon.this.getApplicationContext(), R.string.task_alert_nullUrl, Toast.LENGTH_SHORT).show();
            return;
        }else{}
        if(model==null){
        ...
        model.setTimeOut(Integer.parseInt( et_timeOut.getText().toString().trim().length()==0?"60":et_timeOut.getText().toString().trim()));
        ...
        model.setAddress( et_address.getText().toString().trim().length()==0?"http://":et_address.getText().toString().trim() );
        model.setPort( et_port.getText().toString().trim().length()==0? 80:Integer.parseInt( et_port.getText().toString().trim() ) );
        model.setUser( et_user.getText().toString() );
        model.setPass( et_pass.getText().toString() );*/
