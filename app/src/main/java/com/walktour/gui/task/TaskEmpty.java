...
public class TaskEmpty extends BaseTaskActivity {
	TaskListDispose taskd = null;
	TaskEmptyModel model = null;
	private String tag = "TaskEmpty";
			...
			if (RABTAG.equals(super.getRabTag())) {
				for (int i = 0; i < taskd.getCurrentTaskList().size(); i++) {
					if (taskd.getCurrentTaskList().get(i).getTaskID().equals(super.getModifyBefRabName())) {
						model = (TaskEmptyModel) ((TaskRabModel) (taskd.getCurrentTaskList().get(i))).getTaskModel()
								.get(taskListId);
						break;
					}
				}
			} else {
				model = (TaskEmptyModel) taskd.getTaskListArray().get(taskListId);
			}
			abstModel = model;
			isNew = false;
		}
	...
	private void showView() {
		...
		final View textEntryView = factory.inflate(R.layout.task_empty, null);
		((TextView) textEntryView.findViewById(R.id.title_txt)).setText(R.string.act_task_empty);// 设置标题
		...
		if (model != null) {
			...
			keepTimeEditText.setText(String.valueOf(model.getIdleTestConfig().getKeepTime()));
		} else {
			keepTimeEditText.setText("60");
			taskNameEditText.setText("Idle");
		}
		// 保存任务
	...
	public void saveTestTask() {
		if (taskNameEditText.getText().toString().trim().length() == 0) { // 任务名为空
			Toast.makeText(com.walktour.gui.task.TaskEmpty.this.getApplicationContext(), R.string.task_alert_nullName,
		...
		if (keepTimeEditText.getText().toString().trim().length() == 0) {
			...
			model.getIdleTestConfig().setKeepTime(Integer.parseInt(keepTimeEditText.getText().toString().trim()));
		}
	...
	/*
	 * private void turnToTaskList(){ Intent intent = new
	 * Intent(com.walktour.gui.task.TaskEmpty.this ,Task.class);
	 * startActivity(intent); com.walktour.gui.task.TaskEmpty.this.finish(); }
	 */
}
