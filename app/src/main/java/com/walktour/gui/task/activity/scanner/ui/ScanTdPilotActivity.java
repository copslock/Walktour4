 ...
 * Scanner TdTopN界面
 */
public class ScanTdPilotActivity extends BasicActivity  implements RefreshEventListener{
	...
	private ArrayList<TdScdmaPilotModel> tdModelResult;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tdModelResult = TraceInfoInterface.traceData.getScanResultList(ScanIDShow.SCANID_TDSCDMA_PCCPCH);
		initView();
		RefreshEventManager.addRefreshListener(this);
	}

	private void initView() {
		setContentView(createChart(tdModelResult == null ? 0 : tdModelResult.size(), 4));
	}
	 ...
	 private LinearLayout createChart(int dataCount, int type) {
	    	...
	    	showColumnNameLineTwo = getColumnNameLineTwo(type);
	 	...
	 	private String[] getColumnNameLineOne(int type) {//
	 		...
	 		if (type == 1 || type == 2 || type == 3)
	 			...
	 			showColumnNameLineOne = new String[]{"UARFCN", "CPI", "RSCP", "ISCP", "Ec/Io"};
	 		else if (type == 5)
	    ...
	    private List<HashMap<String, Object>> getItemValueLineOneTypeOne(int count,String[] showColumnNameLineOne) {
	    	...
	    	for (int i = 0; i < count; i++) {
				...
				item.put(showColumnNameLineOne[0], UtilsMethod.convertValue(tdModelResult.get(i).getChannel()));
				item.put(showColumnNameLineOne[1], UtilsMethod.convertValue(tdModelResult.get(i).getMidambleCode()));
				item.put(showColumnNameLineOne[2], UtilsMethod.convertValue(tdModelResult.get(i).getfPCCPCHRSCP()));
				item.put(showColumnNameLineOne[3], UtilsMethod.convertValue(tdModelResult.get(i).getfPCCPCHISCP()));
				item.put(showColumnNameLineOne[4], UtilsMethod.convertValue(tdModelResult.get(i).getfPCCPCHEcIo()));
				item.put("color", 0);
...
//					item.put("color", Color.RED);
	  ...
	  //======================================DATA LINE TWO=======================================
	    ...
	    private List<HashMap<String, Object>> getItemValueLineTwoTypeTwo(int count, String[] showColumnNameLineTwo) {
	    	...
	    	for (int i = 0; i < count; i++) {
	    		HashMap<String, Object> item = new HashMap<String, Object>();
				item.put(showColumnNameLineTwo[0], UtilsMethod.convertValue(tdModelResult.get(i).getSyncID()));
				item.put(showColumnNameLineTwo[1], "");
				item.put(showColumnNameLineTwo[2], "");
				item.put(showColumnNameLineTwo[3], UtilsMethod.convertValue(tdModelResult.get(i).getfPCCPCHSIR()));
				list.add(item);
	    	}
			...
			switch (refreshType) {
	         case ACTION_WALKTOUR_TIMER_CHANGED:
	             if(!ApplicationModel.getInstance().isFreezeScreen()){
	            	 ...
	            	 tdModelResult = TraceInfoInterface.traceData.getScanResultList(ScanIDShow.SCANID_TDSCDMA_PCCPCH);
	            	 dataList = getDataLineOne(tdModelResult == null ? 0 : tdModelResult.size(), showColumnNameLineOne, 4);
	            	 chart.setItemValueLineOne(dataList);
	            	 chart.setItemValueLineTwo(getDataLineTwo(tdModelResult == null ? 0 : tdModelResult.size(), showColumnNameLineTwo, 4));
