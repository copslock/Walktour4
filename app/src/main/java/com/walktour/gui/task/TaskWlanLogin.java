...
public class TaskWlanLogin extends BaseTaskActivity {
	TaskListDispose taskd = null;
	TaskWlanLoginModel model = null;
	...
	private Context context=TaskWlanLogin.this;
			...
			if (RABTAG.equals(super.getRabTag())) {
				for (int i = 0; i < taskd.getCurrentTaskList().size(); i++) {
					if (taskd.getCurrentTaskList().get(i).getTaskID().equals(super.getMultiRabName())) {
						model = (TaskWlanLoginModel) ((TaskRabModel) (taskd.getCurrentTaskList().get(i))).getTaskModel()
								.get(taskListId);
						break;
					}
				}
			} else {
				model = (TaskWlanLoginModel) taskd.getTaskListArray().get(taskListId);
			}
			abstModel = model;
			isNew = false;
		}
	...
	private void findView() {
		...
		(initTextView(R.id.title_txt)).setText(R.string.act_task_wlanlogin);// 设置标题
		...
		tv_timeOut.setText(getString(R.string.task_wlanlogin_timeout));
		...
		if (model != null) {
			...
			taskNameEditText.setText(getString(R.string.act_task_wlanlogin));
	...
	private void initWifiInfo(){
		if (null != model) {
			String apName = model.getWlanWebLoginTestConfig().getApName();
			...
			wifiUserET.setText(model.getWlanWebLoginTestConfig().getWlanAccount().getUsername()+"");
			wifiPasswordET.setText(model.getWlanWebLoginTestConfig().getWlanAccount().getPassword()+"");
		}
	}
	...
	public void saveTestTask() {
		if (taskNameEditText.getText().toString().trim().length() == 0) { // 任务名为空
			Toast.makeText(com.walktour.gui.task.TaskWlanLogin.this.getApplicationContext(),
		...
		if (((Long) dataConnectType.getSelectedItemId()).intValue() == 1) // 只有是WIFI的才设置
		  ...
		  if(((Long)dataConnectType.getSelectedItemId()).intValue()==1){
	        	model.setTypeProperty(WalkCommonPara.TypeProperty_Wlan);
	        	model.getWlanWebLoginTestConfig().setApName(wifiSSIDET.getText().toString()+"");
	        	model.getWlanWebLoginTestConfig().getWlanAccount().setUsername(wifiUserET.getText().toString()+"");
	        	model.getWlanWebLoginTestConfig().getWlanAccount().setPassword(wifiPasswordET.getText().toString()+"");
	        }
