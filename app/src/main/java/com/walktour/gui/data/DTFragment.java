...
public class DTFragment extends FragmentBase implements onDismissListener{
	...
	private List<DataModel> localList = new ArrayList<DataModel>();
	
	public DTFragment() {
		FLAG = TestType.DT.name();
	}
...
//		HashMap<String, ArrayList<String>> map = getAllFilterCondiction(FLAG);
		...
		if (local) {
			...
			localList.clear();
//			list = mDbManager.getFiles(false, FLAG, map);
...
//			parentDataModel.titleOrder = list.get(i).workOrderNum;
	...
	private boolean[] getSettingState() {
		...
		states[3] = sp.getBoolean(FilterKey.KEY_IS_DISPLAY_SETTING + FLAG, false);
		return states;
	}
