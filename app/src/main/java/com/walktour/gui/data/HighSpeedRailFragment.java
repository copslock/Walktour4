...
import com.walktour.gui.data.dialog.HighSpeedRailView;
 ...
 *
 */
public class HighSpeedRailFragment extends FragmentBase implements onDismissListener{
	...
	private PopDialogView mLineDialog;
	private HighSpeedRailView highSpeedRailView;
	...
	public HighSpeedRailFragment() {
		FLAG = SceneType.HighSpeedRail.name();
	}
	@Override
	public void onStart() {
		super.onStart();
	...
	OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View view) {
			...
			case R.id.Button03://无效过滤dialog
				...
				break;
			case R.id.Button_other://高铁线路dialog
				if (highSpeedRailView.enable()) {
					mLineDialog.showAsDropDown(view, 0, DensityUtil.dip2px(mActivity, 240));//DensityUtil.dip2px(mActivity, 300)
				} else {
					mLineDialog.showAsDropDown(view, 0, 0);
				}
	...
	private void initButtons(final View root) {
		...
		other.setText(getResources().getString(R.string.highspeed_route));
	...
	private void initAllPopWindows() {
		...
		mLineDialog = new PopDialogView(mActivity);
		...
		mLineDialog.setOnDismissListener(this);
	...
	private void initFloorView() {
		highSpeedRailView = new HighSpeedRailView(mActivity, FLAG);
		View v = highSpeedRailView.getView();
		mLineDialog.setView(v);
		highSpeedRailView.setClickListenerCallBack(new ClickListenerCallBack() {
			
			@Override
			public void onSummit() {
				mLineDialog.closeView();
				refreshData();
			}
