 ...
 *
 */
public class UnicomDataTransfer extends BaseDataTransfer implements OnProgressChangeListener {
	/** FTP上传工具类 */
	private FtpOperate mFtpUtils = null;
	/** FTP服务类 */
	private FtpServerModel mFtpServer;

	public UnicomDataTransfer(DataTransService service) {
		super(ServerManager.SERVER_FTP, "UnicomDataTransfer", service, "ftp_upload.log");
	}
	...
	protected void uploadCurrentFileType() {
		...
		case File_Exits:
			try {
				if (super.mCurrentFileType == FileType.DCF) {
					mFtpUtils.renameFileName(remoteFile, remoteFile.replace(".tmp", ".dcf"));
					LogUtil.i(TAG, "dcf file rename success");
				}
