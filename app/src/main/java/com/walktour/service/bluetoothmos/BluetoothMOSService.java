 ...
 *
 */
public class BluetoothMOSService extends Service {
	/** 日志标识 */
	private static final String TAG = "BluetoothMOSService";
	/** 传递的关键字：是否要计算PESQ */
	public static final String EXTRA_KEY_CALCULATE_PESQ = "calculate_pesq";
	/** 传递的关键字：主叫还是被叫 */
	public static final String EXTRA_KEY_MOS_TYPE = "mos_type";
	/** 传递的关键字：蓝牙MOS头对象 */
	public static final String EXTRA_KEY_BLUETOOTH_MOS = "bluetooth_mos";
    ...
    /** 反馈的消息类型：搜索完毕 */
	public static final int EXTRA_SEARCH_FINISHED = 9;
	/** 当前设备设备是否插入耳机广播 */
	...
	private final BroadcastReceiver mBluetoothReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
				...
				mFindDevices.clear();
//				setState(BluetoothMOSService.State.find_device_doing);
			} else if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action)) {
				LogUtil.d(TAG, "-----------ACTION_DISCOVERY_FINISHED------------");
				if (mFindDevices.isEmpty())
					setState(BluetoothMOSService.State.find_device_fail);
				else
					setState(BluetoothMOSService.State.check_device);
				handlerMessage(EXTRA_SEARCH_FINISHED,"finished");
				...
				if (device.getName().startsWith(DEVICE_NAME_DEFAULT)) {
					...
					BluetoothMOSDevice bmd = new BluetoothMOSDevice(device, BluetoothMOSService.this, rssi);
	...
	public String convertAudioFile(String recordFilePath, int sampleRate) {
		LogUtil.d(TAG,"recordFilePath:" + recordFilePath);
	...
	public IBinder onBind(Intent intent) {
		...
		this.mCurrDevice = intent.getParcelableExtra(EXTRA_KEY_BLUETOOTH_MOS);
		if (this.mCurrDevice != null) {
			this.mCurrDevice.setService(this);
	...
	private final IBluetoothMOSServiceBinder.Stub mBinder = new IBluetoothMOSServiceBinder.Stub() {
		...
		public boolean runPlayback(String fileType) throws RemoteException {
			...
			if (mCurrDevice != null)
				return mCurrDevice.runPlayback(BaseCommand.FileType.getType(fileType));
			return false;
		}
		...
		public boolean runRecord(String fileType) throws RemoteException {
			...
			if (mCurrDevice != null)
				return mCurrDevice.runRecord(BaseCommand.FileType.getType(fileType));
			return false;
		}
		...
		public boolean initMOS(String fileType, boolean isPlayer) throws RemoteException {
			...
			if (mCurrDevice != null) {
				return mCurrDevice.initMOS(BaseCommand.FileType.getType(fileType), isPlayer);
			}
			return false;
		}
