...
import com.dingli.dmplayer.sdktest.YoutubePlayer;
 ...
 *
 */
public class VideoPlayYoutubeActivity extends Activity implements OnClickListener {
	/** 日志标识 */
	private static final String TAG = "VideoPlayYoutubeActivity";
	/** 视图界面 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_dmplayer_youtube);
		this.findView();
		mPlayer = new YoutubePlayer(mWebView, new VideoPlayEventListener(this), getApplicationInfo().uid,
	...
	private void regeditBroadcast() {
		...
		intentFilter.addAction(VideoPlayTestBase.BROADCAST_START);
		intentFilter.addAction(VideoPlayTestBase.BROADCAST_STOP);
		intentFilter.addAction(VideoPlayTestBase.BROADCAST_QUIT);
		intentFilter.addAction(VideoPlayTestBase.BROADCAST_FIRST_DATA_ARRIVED);
	...
	private void startTest() {
		...
		info.prebuffer_time_ms = 2000;
		info.rebuffer_time_ms = 2000;
		mPlayer.StartTest(info);
	}
	...
	private class MyReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(VideoPlayTestBase.BROADCAST_STOP)
					|| intent.getAction().equals(VideoPlayTestBase.BROADCAST_QUIT)) {
				finish();
			} else if (intent.getAction().equals(VideoPlayTestBase.BROADCAST_START)) {
				startTest();
			} else if (intent.getAction().equals(VideoPlayTestBase.BROADCAST_FIRST_DATA_ARRIVED)) {
