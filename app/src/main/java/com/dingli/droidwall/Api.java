...
//	public static DroidApp applications[] = null;
	...
	private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, boolean showErrors) {
		...
		final boolean whitelist = !blacklist;
...
//		}
		...
		try {
			...
			final SparseArray<DroidApp> map = new SparseArray<DroidApp>();
			final Editor edit = prefs.edit();
...
//			}
			...
			DroidApp[] applications = new DroidApp[map.size()];
			for(int i = 0;i < map.size();i++){
				applications[i] = map.valueAt(i);
			}
			return applications;
		} catch (Exception e) {
			alert(ctx, "error: " + e);
